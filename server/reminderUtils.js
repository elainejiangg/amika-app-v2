...
// export async function getGPTSummary(relationName) {
//   const response = await openai.createCompletion({
//     model: "text-davinci-003",
//     prompt: `Summarize the relation: ${relationName}`,
//     max_tokens: 50,
//   });
...
export async function scheduleEmail(reminder, googleId, userInfo) {
  ...
  occurrences.forEach((occurrence) => {
    ...
    cron.schedule(cronTime, async () => {
      ...
      //   const summary = await getGPTSummary(relationName);
      //   const emailBody = `Method: ${method}\nRelation: ${relationName}\nSummary: ${summary}`;
